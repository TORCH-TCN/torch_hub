...
def flow_template(specimen, app_config): 
    ...
    try:
        ...
        logger.info(f"Flow using {specimen.name}, Flow ID {flow_run_id}, current state {flow_run_state}, example flow config {example_config}...")
        ...
        logger.error(f"Error running flow_template")
