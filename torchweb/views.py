...
def settings(workflowid = 1):
    if request.method == 'POST':
        ...
        folder_increment = WorkflowSettings.query.filter_by(name = 'folder_increment').first()
        ...
        output_base_path = WorkflowSettings.query.filter_by(name = 'output_base_path').first()
    ...
    return render_template("settings.html",user=current_user,settings=settings,filetypes = filetypes, workflow = workflow, categories=categories)
